...
class DiscountApp(AppTemplate):
    ...
    def check_active_discounts_on_all_levels(self, product_df: pd.DataFrame,
                                             discount_level: str,
                                             discount_identifier: str) -> bool:
        res = []

        res.append(self.check_active_discount(discount_level=discount_level,
                                              discount_identifier=discount_identifier))
        ...
        for current_level, levels_to_check in [
            (DiscountLevel.product_name.name, [DiscountLevel.product_category.name,
                                               DiscountLevel.manufacturer.name]),
            (DiscountLevel.product_category.name,
             [DiscountLevel.manufacturer.name])
        ]:
            if discount_level == current_level:
                for product_attribute in levels_to_check:
                    identifier = product_df.loc[discount_level_condition,
                                                product_attribute].values[0]
                    res.append(self.check_active_discount(
                        discount_level=product_attribute,
                        discount_identifier=identifier))
    ...
    def check_active_discount(self, discount_level: str, discount_identifier: str) -> bool:
        ...
        if active_discount_df.shape[0] > 0:
            st.write(
                f'Active discount on {discount_level} {discount_identifier}')
